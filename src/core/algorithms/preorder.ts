...
const generator = function* preorderWalk(graph?: GraphData) {
  ...
  function* traverse(nodeId: number): Generator<any, void, any> {
    ...
    yield { type: 'visitedNode', node: nodeId, log: `Log node ${nodeObj.label} value (Pre-order visit)`, vars: { traversed: path.join(' -> ') } };
  ...
  yield* traverse(1);
  yield { type: 'done', log: `Preorder walk completed: ${path.join(' -> ')}` };
};

export const preorderAlgo: Algorithm = {
  id: 'preorder',
